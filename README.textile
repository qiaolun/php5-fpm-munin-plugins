...
<pre><code>pm.status_path = /fpm-status</code></pre>
...
<pre><code>location ~ ^/(fpm-status|fpm-ping)$ {
...
You'll need to make sure that from within your box, you can curl /fpm-status with # curl http://localhost/fpm-status. You should get a response similar to this:
...
* env.url: Set a custom url, defaults to _http://127.0.0.1/fpm-status_
